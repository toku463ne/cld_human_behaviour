...
func main() {
	...
	// And the ground's taste in builds (stage 57c), which is the same rule
	...
	// Or only the half of it that money is charged by (stage 80a): a hand is
	// taken up by what weighs something, so a coin takes none and a body can
	...
	// sits at the seller's floor. Every figure in HISTORY.md for stages 80
	// and 82 was measured here.
	if *notaste {
		cfg.TrinketTaste, cfg.AdornNeedsSurvival = 0, false
