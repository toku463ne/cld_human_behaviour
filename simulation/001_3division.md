...
- **結末B**: **tick 857,089 でもまだ152体**。**349世代**まで進み、**理性が 6.8 まで削られている**
...
> **この記録のあと、見本の地図は変わりました**（2026-09-20 に地域タイルへ `rich` を載せた＝西1.6 / 北東0.6 / 南東1.0）。**上の2つの実行を再現するには、記録どおり `git checkout 928a227` で当時の地図を使ってください。** 今の地図は同じ地形・同じ地域ですが、**食料の寄り方が違います**（西に立つ割合 0.61 → 0.73）。
...
## 結末B——85万tick 後もまだ152体（`7452d18` の地図・シード1・tick 857,089）

![tick 857089 の画面](001_3division-857089.png)

```
tick 857089 (hour 0.08)  pop 152 (m 85 / f 67)  food 13  births 39663  deaths 39718 (kills 28216, drowned 1932)  gen 349
avg power 42.6  rationality 6.8  intelligence 91.4  vitality 118.9  hunger 34.5
```

（**上はヘッドレスで再現した値**。画面から読んだときは **pop を 154、births を 33,663、deaths を 33,718** と読んでいたが、**節点の丸が桁に重なっていて 2/4 と 3/9 を読み違えていた**。結末Aでも同じ読み違いをしている＝**この画面の数字は目で読まず、必ずヘッドレスで取り直すこと**。）

**シードは結末Aと同じ 1。違うのは地図の版だけで、差は32セル**（四隅の高地を荒れ地にしたかどうか）。**ヘッドレスで `7452d18` の地図・シード1・820 × 660 を回すと、tick 857,089 の画面と一致する**:
...
| 600,000 | 143 | **7** | 27,684 | 240 | 7.9 |
| 857,089 | 152 | 13 | 39,663 | 349 | **6.8** |
...
- **個体数 150 前後で回り続けている。** **`food 13`**——**地面の食料はほぼ常に食べ尽くされている**＝**食料の上限が個体数の上限になっている**（結末Aの、落ちる直前と同じ状態が、落ちずに続いている）。
- **349世代。** 結末Aは21世代で止まった。**この世界だけが「長期の選択圧」を実際に受けている。**
- **理性が 6.8 まで削られている。** **同じ世界の 50,000tick 時点は 19.7** で、**上下しながら下がり続けた**（19.7 → 12.1 → 22.9 → 10.0 → 7.9 → 6.8）。**`CLAUDE.md` の「理性の選択圧の弱さ」に、初めて長時間の材料がついた**——**弱いのではなく、この条件では負に効いている**可能性がある。**ただし1シードなので、揺らぎと区別がついていない。**
- **体力は高い**（`vitality 118.9`）。**理性を削って体力に回した配分**、という読みができる（予算制なので配分は動かせても合計は増えない）。
- **暴力的**: 39,718死のうち **kills 28,216（71%）**。**溺死1,932** も結末A（130）の15倍で、**個体数が多いぶん川を渡る回数も多い**。
...
| births | 2,005 | **39,663** |
| deaths | 3,836 | 39,718 |
| kills（死因に占める割合） | 1,491（39%） | 28,216（71%） |
| drowned | 130 | 1,932 |
| gen | 21 | **349** |
| avg power | 80.2 | 42.6 |
| avg rationality | 65.0 | **6.8** |
| avg vitality | 60.8 | 118.9 |
| avg hunger | 71.3 | 34.5 |
...
- **そして20,000tick では、理性が 19.7 → 6.8 まで下がる過程を1度も見ていない**（結末Bの世界。50,000tick から 857,089tick にかけて）。**能力の選択圧を語るなら、この時間帯を一度は測るべき。**
...
git checkout 7452d18 && go run ./cmd/devview -tiled tiled/samples/001_3division.tmj   # 結末B（シードも1）
```
